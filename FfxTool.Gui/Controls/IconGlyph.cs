...
namespace FfxTool.Gui
{
    ...
    /// <summary>
    /// The miniature of the settings switch — a 22×12 primary pill with a
    /// white dot parked on the right (ON) or a ghost pill with the dot on
    /// the left (OFF). The app's every former checkmark that wasn't a
    /// real control now reads as this: the selected palette swatch's
    /// badge, the Plugin Profiles "Profile linked" badge, everywhere a
    /// state used to be spelled in ✓ glyphs. Colors live on resource KEYS
    /// so the pill re-themes with the palettes like everything else.
    /// </summary>
    public sealed class MiniPill : Border
    {
        ...
        public MiniPill()
        {
            ...
            Child = new Grid();
            ((Grid)Child).Children.Add(_dot);
            Set(true);
        }

        /// <summary>ON: filled primary track, dot right. OFF: ghost track
        /// (highest surface + outline hairline), dot left, dimmed.</summary>
        public void Set(bool on)
        {
            if (on)
            {
                SetResourceReference(BackgroundProperty, "B.Primary");
                BorderThickness = new Thickness(0);
                _dot.HorizontalAlignment = HorizontalAlignment.Right;
                _dot.Margin = new Thickness(0, 0, 2, 0);
                _dot.SetResourceReference(System.Windows.Shapes.Ellipse.FillProperty, "B.OnPrimary");
                Opacity = 1;
            }
            else
            {
                SetResourceReference(BackgroundProperty, "B.SCHighest");
                BorderBrush = null;
                SetResourceReference(BorderBrushProperty, "B.OutlineVariant");
                BorderThickness = new Thickness(1);
                _dot.HorizontalAlignment = HorizontalAlignment.Left;
                _dot.Margin = new Thickness(2, 0, 0, 0);
                _dot.SetResourceReference(System.Windows.Shapes.Ellipse.FillProperty, "B.OnSurfaceVariant");
                Opacity = 0.75;
            }
        }
    }
